...
//    static std::string     snake_head[no_snake_head] = {"./textures/lib2/snake_head/snake_skull.bmp"};
    ...
    while (this->backgrounds[++this->background])//++this->background < no_backgrounds)
        if (!this->bg_textures[this->background].loadFromFile("./textures/lib2/backgrounds/" + static_cast<std::string>(this->backgrounds[this->background]))) //|| !this->bg_textures[1].loadFromFile("../resources/algorithm.jpg") || !this->bg_textures[2].loadFromFile("../resources/space.jpg"))//, sf::IntRect(0, 0, 640, 480)))//if (!texture.loadFromFile("../Crab Nebula.bmp"))//also works
            featured_renderer::error_message("SFML texture Fails to load");
    while (this->snake_foods[++this->food])
        if (!this->food_textures[this->food].loadFromFile("./textures/lib2/food/" + static_cast<std::string>(this->snake_foods[this->food])))
            featured_renderer::error_message("SFML NO food texture");
    while (this->snake_heads[++this->snake_head])
        if (!this->snake_head_textures[this->snake_head].loadFromFile("./textures/lib2/snake_head/" + static_cast<std::string>(this->snake_heads[this->snake_head])))//1
            featured_renderer::error_message("SFML NO snake skull texture");
    while (this->snake_skins[++this->snake])
        if (!this->snake_textures[this->snake].loadFromFile("./textures/lib2/snake/" + static_cast<std::string>(this->snake_skins[this->snake])))//2
    ...
    this->background = rand() % this->background;
    this->snake_head = rand() % this->snake_head;
    this->snake = rand() % this->snake;
    this->food = rand() % this->food;
}
